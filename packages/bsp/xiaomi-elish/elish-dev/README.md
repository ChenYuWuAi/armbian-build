...
设备树 `battery-pack` = **4.45V / 8600mAh / 33.2Wh**，两颗表计各 4.3Ah/16.6Wh，
原厂 `qcom,fv-max-uv = 4500000`（4.5V 满充）⇒ 这块电池是 **1S2P（单串并联）**，
不是 2S。因此原厂 `dual_fuel_gauge_class.c` 的合成规则是：

| 量 | 原厂函数 | 规则 |
|---|---|---|
| 电压 | `fg_read_volt()` | `MAX(两芯)` |
| 电流 | `fg_read_current()` | 两芯**之和** |
| 容量 | `fg_read_system_soc()` | 按两颗 FCC **加权平均** |
| 温度 | `fg_read_temperature()` | 任一颗 ≤ **15.0℃** 取 `MIN`，否则取 `MAX` |
| 功率 | — | 电压 × 电流 |

- **daemon（热控）**：`battery_read()` 已按上表实现（`--read` 可现场核对）。
  **重要 bug 修复**：原先 daemon 只读 `gauge0` 的温度、还把它错写成容量再覆盖，
  等于拿**较凉的那颗**做热控——实测两芯能差 4℃，热的那颗到 46℃ 仍满功率充，
  这才是"温控没效果"的真正原因。
- **聚合模块（BAT0，给 Vitals/btop/upower 看）**：原先电压取两芯之和（8.4V）、
  电流取平均、设计容量继承单芯 → 全部错。现按上表改为电压 MAX、电流求和、
  容量 FCC 加权、温度 min/max，并把 `of_node` 指向 `battery-pack` 以报出整包
  设计容量（8.6Ah/33.2Wh，原先只有一半）。
